// erreur de deux joueurs avec le même id et/ou port UDP
// revenir dans le chemin principal ?
...
/**
 * L'id correspondant à un joueur
 * @param : le joueur dont on veut récupérer l'id
 */
...
void *read_multidif()
{
  ...
  struct sockaddr_in address_sock;
  address_sock.sin_family = AF_INET;
  address_sock.sin_port = htons(portmul);
  address_sock.sin_addr.s_addr = htonl(INADDR_ANY);
  bind(sock, (struct sockaddr *)&address_sock, sizeof(struct sockaddr_in));
  struct ip_mreq mreq;
  mreq.imr_multiaddr.s_addr = inet_addr("225.103.207.100");
...
void *jouer(void *jo)
{
  ...
  if (r != -1)
  {
    ...
    size_rec = recv(descr, buff, 200 * sizeof(char), 0);
    buff[size_rec] = '\0';
    printf("%s\n", buff);
    char *s = strtok(buff, " ");
    while (strcmp(s, "WAITING***"))
    {
      ...
      if (!already_verified && (!strcmp(m, "NEWPL") || !strcmp(m, "REGIS")))
      {
        char *n;
        int p;
        flag = 0;
